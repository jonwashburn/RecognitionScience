...
/-! Ledger, atomic tick, conservation, and early theorems (T2/T3). -/
...
/-- Edge potential on the ledger (debit − credit). -/
def phi {M} (L : Ledger M) : M.U → ℤ := fun u => L.debit u - L.credit u

/-- Flux along a chain equals the potential difference. -/
...
/-- T3 (Continuity/Conservation): flux vanishes on closed chains. -/
